/*
 * Motor Control — Duty-cycle drive with overcurrent protection
 *
 * Direct duty-cycle control with hall-based six-step commutation.
 * The host controller closes any higher-level loops externally.
 *
 * Overcurrent protection:
 *   Hard fault at CURRENT_LIMIT_MA — immediate brake, requires clear.
 *   Soft limit at torque_limit_ma  — proportional duty pullback.
 */

#include "ms51_reg.h"
#include "ms51_config.h"
#include "motor.h"
#include "commutation.h"
#include "hall.h"
#include "pwm.h"
#include "adc.h"

static motor_state_t state;
static fault_code_t  fault;
static int16_t       target_duty;
static uint16_t      torque_limit_ma;
static uint16_t      current_ma;

void motor_init(void)
{
    state = MOTOR_IDLE;
    fault = FAULT_NONE;
    target_duty = 0;
    torque_limit_ma = DEFAULT_TORQUE_LIMIT_MA;
    current_ma = 0;
}

void motor_set_duty(int16_t duty)
{
    target_duty = duty;
}

void motor_set_torque_limit(uint16_t ma)
{
    if (ma > CURRENT_LIMIT_MA)
        ma = CURRENT_LIMIT_MA;
    torque_limit_ma = ma;
}

void motor_start(void)
{
    if (state == MOTOR_FAULT)
        return;
    int8_t dir = (target_duty >= 0) ? 1 : -1;
    fault = FAULT_NONE;
    state = MOTOR_RUN;
    commutation_update(dir);
    pwm_enable();
}

void motor_stop(void)
{
    target_duty = 0;
    pwm_set_duty(0);
    commutation_coast();
    pwm_disable();
    state = MOTOR_IDLE;
}

void motor_clear_fault(void)
{
    target_duty = 0;
    fault = FAULT_NONE;
    commutation_coast();
    pwm_disable();
    state = MOTOR_IDLE;
}

motor_state_t motor_get_state(void) { return state; }
fault_code_t  motor_get_fault(void) { return fault; }
uint16_t motor_get_current(void)    { return current_ma; }

void motor_poll_fast(void)
{
    if (!hall_poll())
        return;

    if (state == MOTOR_RUN) {
        int8_t dir = (target_duty >= 0) ? 1 : -1;
        commutation_update(dir);
    }
}

void motor_update(void)
{
    int16_t duty;
    int8_t dir;
    uint16_t abs_duty;
    uint8_t hall_state;

    /* ── Sensors ─────────────────────────────────────────────────── */

    hall_poll();
    current_ma = adc_read_current_ma();
    hall_state = hall_read();

    /* ── Faults ──────────────────────────────────────────────────── */

    if (current_ma > CURRENT_LIMIT_MA) {
        pwm_fault_brake();
        state = MOTOR_FAULT;
        fault = FAULT_OVERCURRENT;
        return;
    }

    if (hall_state == 0 || hall_state == 7) {
        pwm_fault_brake();
        state = MOTOR_FAULT;
        fault = FAULT_HALL_INVALID;
        return;
    }

    if (state != MOTOR_RUN)
        return;

    /* ── Duty with current limiting ──────────────────────────────── */

    duty = target_duty;

    if (current_ma > torque_limit_ma) {
        /* Proportional pullback: scale duty so current tracks limit */
        duty = (int16_t)(((int32_t)duty * torque_limit_ma) / current_ma);
    }

    /* ── Apply ───────────────────────────────────────────────────── */

    if (duty >= 0) {
        dir = 1;
        abs_duty = (uint16_t)duty;
    } else {
        dir = -1;
        abs_duty = (uint16_t)(-duty);
    }

    pwm_set_duty(abs_duty);
    commutation_update(dir);
}
